...
file_o = open('%s_D4A8.txt'%counter,'w')
...
for r in range(NUM_COMMUNICATION_ROUND):
    ...
    modelR = model 
    modelA1A2A3A4 = modelR
    modelA1A2 = modelA1A2A3A4
    modelA1 = modelA1A2
    modelA2 = modelA1A2
    modelA3A4 = modelA1A2A3A4
    modelA3 = modelA3A4 
    modelA4 = modelA3A4
    modelA5A6A7A8 = modelR
    modelA5A6 = modelA5A6A7A8
    modelA5 = modelA5A6 
    modelA6 = modelA5A6
    modelA7A8 = modelA5A6A7A8
    modelA7 = modelA7A8 
    modelA8 = modelA7A8
    ...
    modelA1A2A3A4 = utils.fedAVG([modelA1A2.get_weights(),modelA3A4.get_weights()])
    print(r,'AGGREGATOR01020304')
    modelA1A2A3A4.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    global_res = modelA1A2A3A4.evaluate(X_test, Y_test)
    file_o.write('%d,%s,%f,%f\n'%(r,'aggregator01020304',0,global_res[1]))
    ...
    modelA6.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    ...
    A7_models = []
    # A7_models.append(modelA7.get_weights())
    for i in range(8):
        # temp_model = clone_model(modelA7)
        temp_model = modelA7
        LOSS = 'categorical_crossentropy' 
        lr = 0.000001
        OPTIMIZER = SGD(lr=lr, momentum=0.9, nesterov=False) # lr = 0.015, 67 ~ 69%
        ...
        file_o.write('%d,%s,%f,%f\n'%(r,temp_dev,local_res[1],global_res[1]))
        temp_weights = copy.deepcopy(temp_model.get_weights())
        A7_models.append(temp_weights)
    modelA7 = utils.fedAVG(A7_models)
    print(r,'AGGREGATOR07')
    modelA7.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    global_res = modelA7.evaluate(X_test, Y_test)
    file_o.write('%d,%s,%f,%f\n'%(r,'aggregator07',0,global_res[1]))

    A8_models = []
    # A8_models.append(modelA8.get_weights())
    for i in range(7):
        # temp_model = clone_model(modelA8)
        temp_model = modelA8
        LOSS = 'categorical_crossentropy' 
        lr = 0.000001
        OPTIMIZER = SGD(lr=lr, momentum=0.9, nesterov=False) # lr = 0.015, 67 ~ 69%
        ...
        file_o.write('%d,%s,%f,%f\n'%(r,temp_dev,local_res[1],global_res[1]))
        temp_weights = copy.deepcopy(temp_model.get_weights())
        A8_models.append(temp_weights)
    modelA8 = utils.fedAVG(A8_models)
    print(r,'AGGREGATOR08')
    modelA4.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    global_res = modelA8.evaluate(X_test, Y_test)
    file_o.write('%d,%s,%f,%f\n'%(r,'aggregator08',0,global_res[1]))
    
    modelA7A8 = utils.fedAVG([modelA7.get_weights(),modelA8.get_weights()])
    print(r,'AGGREGATOR0708')
    modelA7A8.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    global_res = modelA7A8.evaluate(X_test, Y_test)
    file_o.write('%d,%s,%f,%f\n'%(r,'aggregator0708',0,global_res[1]))

    modelA5A6A7A8 = utils.fedAVG([modelA5A6.get_weights(),modelA7A8.get_weights()])
    print(r,'AGGREGATOR05060708')
    modelA5A6A7A8.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    global_res = modelA5A6A7A8.evaluate(X_test, Y_test)
    file_o.write('%d,%s,%f,%f\n'%(r,'aggregator05060708',0,global_res[1]))
    
    model = utils.fedAVG([modelA1A2A3A4.get_weights(),modelA5A6A7A8.get_weights()])
